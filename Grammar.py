class Grammar:
    ...
    def check_cfg(self):
        ...
        #If a non-terminal matches the starting
        # non-terminal (self.starting_nt) ==> set has_starting_symbol = True.
        for key in self.productions.keys():
            if key == self.starting_nt:
                has_starting_symbol = True
        # each key is a valid non-terminal ?
            if key not in self.non_terminals:
                return False
        if not has_starting_symbol:
            return False
        #RHS, it checks whether it is a valid non-terminal, terminal, or epsilon.
        for production in self.productions.values():
            for rhs in production:
                for value in rhs:
...
#any non-terminal in the grammar has a length greater than 1
    def cfg_check(self):
